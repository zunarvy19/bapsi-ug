      ...
      <div class="mt-20">
          ...
          <section class="bg-white border rounded-lg mb-5">
              <div class=" px-4 py-8 mx-auto">
                  ...
                  <form action="/admin/artikel/edit-artikel/{{ $artikel->slug }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="flex flex-col gap-4 mb-4 sm:mb-5">
                        <div class="sm:col-span-2 w-full">
                            ...
                            <input type="text" name="title" id="title" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " value="{{ old('title', $artikel->title) }}" placeholder="Judul" required>
                            @error('title')<p class="peer-invalid:visible text-red-700">{{ $message }}</p>@enderror
                        </div>
                        <div class="w-full">
                            <label for="slug" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Slug</label>
                            <input type="text" name="slug" id="slug" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="Slug" value="{{ old('slug', $artikel->slug) }}" readonly>
